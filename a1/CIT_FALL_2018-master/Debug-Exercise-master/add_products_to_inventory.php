...
unset($_SESSION['aryProductsArray']); //commonize session variable name for products array
...
for ($x=0; $x<=7; $x++) {
	...
	if (!empty($_POST['productname' . $x]))  {
		...
		array_push($aryProductsArray, $produceItem); //commonize session variable name for products array
...
$_SESSION['aryProductsArray'] = serialize($aryProductsArray); //commonize session variable name for products array
session_write_close();
